...
//    print("sansa", sansa.result.head)
    ...
    val aryaFriends = arya.byUsing(_.name).append(AddFriends0)
...
//    prints("arya siblings", aryaSiblings.result)
    ...
    var aryaFriendsOfParents =
      arya
        .byUsing(_.name).append(parents)
        .byUsing(_._2.person.name).append(AddLinked)
//    prints("arya friends of parents", aryaFriendsOfParents.result)
    ...
    /**
      ...
      */
    val addFriendsOfParents =
      parents.andThen(AddLinked).byFirstUsing(_.person.name)
...
//    prints("arya friends of parents 2", aryaFriendsOfParents2.result)
    ...
    val mothersInLaw = spouse.andThen(mothers).byFirstUsing(_.person.name) // ****
    val siblingsMothersInLaw =
      siblings.andThen(mothersInLaw).byFirstUsing(_.person.name) // ****
    val siblingsPeoplesMothersInLaw = // ****
      siblings // ****
        .andThen(AddLinkedPersons()).byFirstUsing(_.person.name)  // ****
        .andThen(mothersInLaw).byFirstUsing(_._2.person.name)  // ****
...
//      arya.byUsing(_.name).append(siblingsPeoplesMothersInLaw).result)
    ...
    val happyMothersDay =
      ...
      users.byUsing(_.name)
        .append(mothersInLaw).map { case (a, (_, b)) =>
          (a, b.copy(hasAs = "MOTHER IN LAW"))
        }

    // Note that these work on arbitrary queries!
    val menWithLongNames = users.filter { user =>
      user.sex === "M" && user.name.length >= 6
    }
    val fromJustTheMen =
      menWithLongNames.byUsing(_.name).append(mothers) ++
      menWithLongNames.byUsing(_.name)
        .append(mothersInLaw).map { case (a, (_, b)) =>
          (a, b.copy(hasAs = "MOTHER IN LAW"))
        }

//    prints("Happy mother's day wishes", happyMothersDay.result)
//    prints("Happy mother's day wishes from just the men",
